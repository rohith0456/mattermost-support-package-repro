...
// imageRef returns image prefixed with registry when registry is non-empty.
//
//	imageRef("", "postgres:15-alpine")                    → "postgres:15-alpine"
//	imageRef("registry.internal:5000", "postgres:15-alpine") → "registry.internal:5000/postgres:15-alpine"
func imageRef(registry, image string) string {
	if registry == "" {
		return image
	}
...
func (g *Generator) generateCompose() (string, error) {
	...
	if p.Services.Database.Enabled {
		if p.Services.Database.Type == "mysql" {
			sb.WriteString(mysqlService(p.Services.Database.ExposedPort, registry))
		} else {
			sb.WriteString(postgresService(p.Services.Database.ExposedPort, registry))
		}
	}
	...
	if p.Services.Search.Enabled {
		if p.Services.Search.Backend == "elasticsearch" {
			sb.WriteString(elasticsearchService(p.Services.Search.ExposedPort, registry))
		} else {
			sb.WriteString(opensearchService(p.Services.Search.ExposedPort, registry))
		}
	}

	// LDAP
	if p.Services.Auth.LDAPEnabled {
		sb.WriteString(ldapService(registry))
	}

	// Keycloak
	if p.Services.Auth.KeycloakEnabled {
		sb.WriteString(keycloakService(registry))
	}

	// MinIO
	if p.Services.FileStorage.UseMinIO {
		sb.WriteString(minioService(p.Services.FileStorage.ExposedPort, registry))
	}

	// MailHog (always included)
	sb.WriteString(mailhogService(p.Services.Email.ExposedPort, p.Services.Email.UIPort, registry))

	// Mattermost (single or replicas)
	image := imageRef(registry, fmt.Sprintf("%s:%s", p.MattermostImage, p.Services.Mattermost.Tag))
	...
	if p.Topology == "multi-node" && p.NodeCount > 1 {
		...
		sb.WriteString(nginxService(p.NodeCount, registry))
	} else {
	...
	if p.Services.Calls.UseRTCD {
		sb.WriteString(rtcdService(p.Services.Calls.ExposedPort, registry))
	}

	// Prometheus
	if p.Services.Observability.PrometheusEnabled {
		sb.WriteString(prometheusService(p.Services.Observability.PrometheusPort, registry))
	}

	// Grafana
	if p.Services.Observability.GrafanaEnabled {
		sb.WriteString(grafanaService(p.Services.Observability.GrafanaPort, registry))
	}
	...
	if p.Services.Tunnel.NgrokEnabled {
		...
		sb.WriteString(ngrokService(tunnelTarget, p.Services.Tunnel.NgrokAPIPort, registry))
	}
...
func postgresService(port int, registry string) string {
	return fmt.Sprintf(`
  postgres:
    image: %s
...
`, imageRef(registry, "postgres:15-alpine"), port)
}

func mysqlService(port int, registry string) string {
	return fmt.Sprintf(`
  mysql:
    image: %s
    platform: linux/amd64
...
`, imageRef(registry, "mysql:8.0"), port)
}

func elasticsearchService(port int, registry string) string {
	return fmt.Sprintf(`
  elasticsearch:
    image: %s
    platform: linux/amd64
...
`, imageRef(registry, "docker.elastic.co/elasticsearch/elasticsearch:8.11.0"), port)
}

func opensearchService(port int, registry string) string {
	return fmt.Sprintf(`
  opensearch:
    image: %s
    platform: linux/amd64
...
`, imageRef(registry, "opensearchproject/opensearch:2.11.0"), port)
}

func ldapService(registry string) string {
	return fmt.Sprintf(`
  openldap:
    image: %s
...
`, imageRef(registry, "osixia/openldap:1.5.0"), imageRef(registry, "osixia/phpldapadmin:0.9.0"))
}

func keycloakService(registry string) string {
	return fmt.Sprintf(`
  keycloak:
    image: %s
    restart: unless-stopped
...
func minioService(port int, registry string) string {
	return fmt.Sprintf(`
  minio:
    image: %s
    platform: linux/amd64
...
`, imageRef(registry, "minio/minio:latest"), port)
}

func mailhogService(smtpPort, uiPort int, registry string) string {
	return fmt.Sprintf(`
  mailpit:
    image: %s
    restart: unless-stopped
...
`, imageRef(registry, "axllent/mailpit:latest"), smtpPort, uiPort)
}
...
func nginxService(nodeCount int, registry string) string {
	var deps strings.Builder
...
`, imageRef(registry, "nginx:alpine"), deps.String())
}

func rtcdService(port int, registry string) string {
	return fmt.Sprintf(`
  rtcd:
    image: %s
...
`, imageRef(registry, "mattermost/rtcd:latest"), port)
}

func prometheusService(port int, registry string) string {
	return fmt.Sprintf(`
  prometheus:
    image: %s
    platform: linux/amd64
...
`, imageRef(registry, "prom/prometheus:latest"), port)
}

func grafanaService(port int, registry string) string {
	return fmt.Sprintf(`
  grafana:
    image: %s
    platform: linux/amd64
...
`, imageRef(registry, "grafana/grafana:latest"), port)
}

func ngrokService(target string, apiPort int, registry string) string {
	return fmt.Sprintf(`
  ngrok:
    image: %s
    restart: unless-stopped
...
`, imageRef(registry, "ngrok/ngrok:latest"), target, apiPort)
}
